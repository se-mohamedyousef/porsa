...
/**
 * Fetch a single EGX live quote directly from TradingView scanner.
 * This avoids Yahoo dependency for real-time price updates.
 */
export async function fetchTradingViewEgxQuote(symbol) {
  ...
  const response = await retryWithBackoff(
      ...
      }),
    {
      ...
      backoffMultiplier: 2,
      jitter: true,
      context: `TradingView Quote ${sym}`,
    }
  );

  if (!response?.ok) {
    throw new Error(`TradingView quote HTTP ${response?.status || 500}`);
  }
  ...
  const rowSymbol = String(row?.s || "").split(":").pop()?.toUpperCase();
  const values = row?.d || [];
  const price = Number(values[1]);

  if (!row || !rowSymbol || !Number.isFinite(price) || price <= 0) {
    return null;
  }
