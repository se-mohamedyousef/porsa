...
export class BaseAgent {
  ...
  /**
   * Call HuggingFace Groq API with circuit breaker, retry logic, and graceful degradation
   */
  async callHuggingFace(prompt) {
    // Adaptive timeout: shorter for simple requests, longer for complex ones
    const isComplex = prompt.length > 2000 || prompt.includes('TOOL:') || prompt.includes('tool_');
    const baseTimeout = isComplex ? 15000 : 8000; // 15s for complex, 8s for simple
    const adaptiveTimeout = Math.min(baseTimeout, 25000); // Cap at 25s
    ...
    try {
      ...
      // Set timeout and track if it fires
      ...
      return await llmCircuitBreaker.execute(
        async () => {
          return await retryWithBackoff(
            async () => {
              const response = await fetch("https://router.huggingface.co/v1/chat/completions", {
                ...
                body: JSON.stringify({
                  ...
                  messages: [{ role: "user", content: prompt.substring(0, 4000) }],
                  stream: false,
                  max_tokens: 1024,
                  temperature: 0.7,
                }),
            ...
            {
              maxRetries: isTimedOut ? 0 : 2, // Don't retry if timeout happened
              initialDelayMs: 500,
              maxDelayMs: 3000,
      ...
      // Graceful degradation: If timeout, return a partial message instead of throwing
      if (isTimedOut || error.name === 'AbortError') {
        this.logger.warn("LLM call timed out, returning graceful degradation", "callHuggingFace", {
          error: error.message,
          timeout: adaptiveTimeout,
        });
        // Return a partial response that indicates we're using fallback data
        return "[RESPONSE TRUNCATED - API timeout]\n" +
          "Based on available data and analysis tools, please try again or use a simpler query.";
      }
